...
- [ ] Восстановление пароля (forgot/reset) — перенесено в этап 6: без работающей почты
      письмо со ссылкой отправить некуда
- [ ] Временно: сгенерированный пароль показывается в модалке «заявка принята», потому что
      письма ещё нет. На этапе 6 блок убирается
...
- Провайдер письма в Strapi (сейчас дефолтный sendmail) — нужен для forgot/reset password
- Забрать хвосты этапа 5: письмо с паролем после регистрации (и убрать показ пароля
  в модалке), восстановление пароля через `/auth/forgot-password` + `/auth/reset-password`
